...
PROMPT
  ...
  mkdir -p "$ITER_DIR"
  : > "$WORK_DIR/usage-total.json"
  printf '%s' '{"tokens":{"total":0,"input":0,"output":0,"reasoning":0},"cost":0}' > "$WORK_DIR/usage-total.json"
  HAS_USAGE=false
  set +e
  # Best-effort token capture with JSON format; fallback to default on failure/timeout
  timeout 120 opencode run --model opencode/big-pickle --format json "$(cat "$PROMPT_FILE")" > "$RESPONSE_FILE" 2>> "$WORK_DIR/opencode.err"
  JSON_EXIT=$?
  set -e
  if [[ $JSON_EXIT -eq 0 && -s "$RESPONSE_FILE" ]]; then
    node --input-type=commonjs -e "
      ...
      let acc; try { acc = JSON.parse(fs.readFileSync(totalPath, 'utf8')); } catch { acc = {tokens:{total:0,input:0,output:0,reasoning:0},cost:0}; }
      acc.tokens.total += total; acc.tokens.input += input; acc.tokens.output += output; acc.tokens.reasoning += reasoning; acc.cost += cost;
      ...
      let obj; try { obj = JSON.parse(text.slice(start, end + 1)); } catch (err) { console.error('JSON parse failed: ' + err.message); process.exit(1); }
    ...
    " && echo "parsed" > "$ITER_DIR/parsed.ok" && HAS_USAGE=true || {
      echo "json parse failed, falling back to default format" >> "$FEEDBACK_FILE"
      rm -f "$RESPONSE_FILE"
    }
  fi

  # Fallback or re-run with default format if JSON path didn't succeed
